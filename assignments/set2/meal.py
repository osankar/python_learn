...
breakfast time, lunch time, or dinner time. If it’s not time for a meal, don’t output anything
at all. Assume that the user’s input will be formatted in 24-hour time as #:## or ##:##. 
...
For instance, given a time like "7:30" (i.e., 7 hours and 30 minutes), convert should return 
7.5 (i.e., 7.5 hours).
"""
def main():
    time = input("What time is it? ")
    time_number = convert(time)
    if 7.00 <= time_number <= 8.00:
        print("breakfast time")
    elif 12.00 <= time_number <= 13.00:
        print("lunch time")
    elif 18.00 <= time_number <= 19.00:
        print("dinner time")
...
def convert(time):
    ...
    hour =int(hh_mm[0])
    minutes = int(hh_mm[1])
    return float(hour + round(float(minutes/60), 2))
